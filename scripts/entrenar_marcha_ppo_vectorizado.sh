...
if [[ "${BASH_SOURCE[0]}" != "$0" ]]; then
    ...
    echo "[ERROR] Usalo como proceso independiente: bash scripts/entrenar_marcha_ppo_vectorizado.sh"
    return 1
fi

RAIZ_REPO="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
DIRECTORIO_ISAACLAB="$RAIZ_REPO/isaac_lab/IsaacLab"
RUTA_SCRIPT="$RAIZ_REPO/codigo/entrenamiento/entrenar_ppo_marcha_vectorizado.py"
...
echo "[INFO] Lanzando entrenamiento PPO de marcha vectorizado."
