...
try:  # pragma: no cover
    ...
    try:
        from gym.vector.utils import batch_space  # type: ignore
    except Exception:  # pragma: no cover
        batch_space = None

try:  # pragma: no cover
    from isaaclab.envs import DirectRLEnv as _IsaacLabDirectRLEnv
except Exception:  # pragma: no cover
    class _IsaacLabDirectRLEnv:
        pass
...
class EntornoMarchaRslRl(_IsaacLabDirectRLEnv):
    """Entorno vectorizado con API compatible con Gym/Isaac Lab para rsl_rl."""
    ...
    def __init__(
        ...
        obs_low = -torch.inf * torch.ones(self.num_obs, dtype=torch.float32)
        obs_high = torch.inf * torch.ones(self.num_obs, dtype=torch.float32)
        act_low = -torch.ones(self.num_actions, dtype=torch.float32)
        act_high = torch.ones(self.num_actions, dtype=torch.float32)

        self.single_observation_space = gym.spaces.Dict(
            {
                "policy": gym.spaces.Box(
                    low=obs_low.cpu().numpy(),
                    high=obs_high.cpu().numpy(),
                    dtype=np.float32,
                )
            }
        )
        self.single_action_space = gym.spaces.Box(
            low=act_low.cpu().numpy(),
            high=act_high.cpu().numpy(),
            dtype=np.float32,
        )
        ...
        self.action_scale = self.cfg.escala_accion
    ...
    def _create_simulation(self) -> None:
        ...
        self.simulacion = self.simulation
        self.dt = cfg_sim.dt
    ...
    def _map_controlled_joints(self) -> None:
        ...
        self.indices_controlados = torch.tensor(
            self.indices_controlados, dtype=torch.long, device=self.device
        )
    ...
    def _get_observations(self) -> dict[str, Tensor]:
        ...
        return {"policy": self._current_obs}
    ...
    def reset(
        ...
        self._last_extras = {}
        return self._get_observations(), self._last_extras
    ...
    def step(self, action: Tensor) -> tuple[dict[str, Tensor], Tensor, Tensor, Tensor, dict[str, Any]]:
        ...
        return self._get_observations(), reward, terminated, truncated, extras
